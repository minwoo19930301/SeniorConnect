# Three-Minute Demo Plan

Use only synthetic names, contacts, letters, images, and app screens.

## 0:00–0:20 — problem and promise

Show a confusing conventional phone briefly, then the four-button home.

Narration: “Four clear choices: Call, YouTube, Speak, and Camera.”

## 0:20–0:55 — Call

Tap Call, show a generic trusted-contact card, confirm the target, and open the normal dialer. Do not show a person name or claim that a call connected.

## 0:55–1:30 — YouTube

Open a generic YouTube scenario. Explain “This is an advertisement. Your video has not ended. Please wait.” Then show an install warning without pressing anything.

## 1:30–2:00 — Speak

Ask “Is the library open today?” Show web search, one short answer, and visible source links. Then ask for a short story to show that unnecessary search is not used.

## 2:00–2:30 — Camera

Photograph a synthetic appointment letter. Confirm the upload. Hear the date, place, and uncertainty in simple language. Show `Take Again`, `Take Me Home`, and `Stop`.

## 2:30–2:50 — technical proof

Show the short architecture:

```text
speech/photo/state
→ minimum structured input
→ GPT-5.6 proposal or explanation
→ deterministic policy
→ confirmation
→ Android action
```

Show the **implemented recorded/live evaluation runner** denying one prompt-injection case. Fixture lint alone is not safety evidence. Include Codex and GPT-5.6 usage required by the event.

## 2:50–3:00 — close

“It does not take the phone away. It gives one clear next step and keeps a trusted human close.”

## Recording checklist

- Narration and on-screen text are in English for submission.
- Total video is three minutes or less.
- Product, Codex use, and GPT-5.6 use are visibly demonstrated.
- No real personal data, third-party copyrighted media, or unlicensed music appears.
- Scenario fixtures are labeled as fixtures; planned features are not presented as implemented.
